...
const Cart = () => {
  ...
  const { cartList, vaciarCarrito, sumTotal, eliminarProducto } = useCartContext()
  ...
  return (
    <div className='container mt-5'>
            ...
            <tbody>
              {cartList.map(productos => (
                <tr key={productos.id}>
                  ...
                  <br/>
                  <button className="btn btn-eliminar" onClick={()=> eliminarProducto(productos.id)}> X </button>
                </tr>
              ))}
            </tbody>
          </table>
        ) : (
          <div className='text-center empty-cart-message'><b>NO HAY PRODUCTOS EN EL CARRITO</b></div>
        )}
      </div>
      ...
      <br/>
        <div className='text-center form-message'><b>FORMULARIO PARA COMPLETAR COMPRA</b></div>
